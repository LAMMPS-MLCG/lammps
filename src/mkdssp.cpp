...
int VERBOSE = 0;
//MProtein a;
mkdssp::mkdssp(LAMMPS * lmp) : Pointers(lmp), filecount(1)//, a(lmp)
{
	a = new MProtein(lmp);
}

int mkdssp::init(int argc, char* argv[]){
	try
	{
...
int mkdssp::reinit() {
	...
	char output[100];
	sprintf(output, "file%d.dssp", filecount);
	filecount++;

	ofstream outfile(std::string(output).c_str(), ios_base::out|ios_base::trunc|ios_base::binary);
	if (not outfile.is_open())
		throw runtime_error("could not create output file");
