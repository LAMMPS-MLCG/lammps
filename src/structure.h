...
class MResidue : protected Pointers
{
  public:
						MResidue(LAMMPS *,const MResidue& residue);
						MResidue(LAMMPS *,uint32 inNumber, char inTypeCode, MResidue* inPrevious);
						MResidue(LAMMPS *,uint32 inNumber,
...
class MChain : protected Pointers
{
  public:

						MChain(LAMMPS * lmp, const MChain& chain);
						MChain(LAMMPS * lmp, const std::string& inChainID) : mChainID(inChainID),Pointers(lmp){}
						~MChain();
...
class MProtein : protected Pointers
{
  public:
						MProtein(LAMMPS *);
						MProtein(LAMMPS *lmp, const std::string& inID, MChain* inChain);
						~MProtein();
