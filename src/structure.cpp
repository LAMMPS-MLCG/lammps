...
//TODO
// 1. create destructor
// 2. create lookup table for atoms mapping the dssp atoms to lammps atoms
...
MResidue::MResidue(LAMMPS *lmp, uint32 inNumber,
		MResidue* inPrevious, const vector<MAtom>& inAtoms):Pointers(lmp)
	, mPrev(inPrevious)
	, mNext(nullptr)
...
MResidue::MResidue(LAMMPS *lmp, uint32 inNumber, char inTypeCode, MResidue* inPrevious):Pointers(lmp)
	, mPrev(nullptr)
	, mNext(nullptr)
	, mSeqNumber(inNumber)
	, mNumber(inNumber)
...
MResidue::MResidue(LAMMPS *lmp,const MResidue& residue):Pointers(lmp),
...
MChain::MChain(LAMMPS *lmp, const MChain& chain):Pointers(lmp)
{
	MResidue* previous = nullptr;
	
	foreach (const MResidue* residue, chain.mResidues)
	{
		MResidue* newResidue = new MResidue(lmp, *residue);
...
MProtein::MProtein(LAMMPS *lmp) : Pointers(lmp)
	, mResidueCount(0)
	, mChainBreaks(0)
...
MProtein::MProtein(LAMMPS *lmp,const string& inID, MChain* inChain) : Pointers(lmp)
	, mID(inID)
	, mChainBreaks(0)
	, mIgnoredWaterMolecules(0)
	, mNrOfHBondsInParallelBridges(0)
	, mNrOfHBondsInAntiparallelBridges(0)
	, mResidueCount(0)								//not in original code
{
	fill(mParallelBridgesPerLadderHistogram, mParallelBridgesPerLadderHistogram + kHistogramSize, 0);
	fill(mAntiparallelBridgesPerLadderHistogram, mAntiparallelBridgesPerLadderHistogram + kHistogramSize, 0);
	fill(mLaddersPerSheetHistogram, mLaddersPerSheetHistogram + kHistogramSize, 0);
...
void MProtein::AddResidue(const vector<MAtom>& inAtoms)
{
	...
	if (hasN and hasCA and hasC and hasO)
	{
		...
		MResidue* r = new MResidue(lmp, resNumber, prev, inAtoms);
...
MChain& MProtein::GetChain(const string& inChainID)
{
	...
	mChains.push_back(new MChain(lmp,inChainID));
	return *mChains.back();
}
...
void MProtein::UpdateProtein(){
//	cout<<"in protein\n";
	mChains = GetChains();
//	vector<MChain*> chains(GetChains());
	foreach (MChain* chain, mChains)
		chain->UpdateChain();
}

void MChain::UpdateChain(){
//	cout<<"in chain\n";
//	vector<MResidue*> res(chain->GetResidues())
	foreach (MResidue* r, mResidues)
		r->UpdateResidue();
}

void MResidue::UpdateResidue(){
	//MAtom* a;
	//cout<<"in residue\n";
	UpdateLocation(mN);
	UpdateLocation(mCA);
...
void MResidue::UpdateLocation(MAtom& a){
	//cout<<"in location\n";
	int id = a.mSerial;
